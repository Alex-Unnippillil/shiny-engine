...
Before enabling any neural backend, establish the exact upstream revision/notices, immutable model hash, model provenance, permitted runtime use, conversion/derivative rights, redistribution terms, tensor/geometry/color requirements, supported operators and independent correctness fixtures. Record separate physical-hardware evidence before advertising performance. A user-provided DLL is not a rights clearance. No generic DLL loader, extraction path, automatic weight downloader or arbitrary model URL is exposed.

TypeScript 5.8.3 is development-only with npm integrity; Playwright 1.57.0 is test-only. Executable extension assets and shaders are bundled. Windows SDK/C++/WinRT/Direct3D are platform dependencies, not redistributed SDK source. No NeuralScreen source/runtime is included. See `THIRD_PARTY_NOTICES.md` and `neural-lab.md`.
