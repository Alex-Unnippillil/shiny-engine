...
int playbackTest(const std::filesystem::path& root,const std::filesystem::path& fixture,const std::filesystem::path& report){
 ...
 try{
  ...
  {Engine engine(api,nullptr,false,true);frames.attach(engine);Item item{std::filesystem::absolute(fixture).wstring(),L"Synthetic original",false};engine.open(item);
   ...
   {NrProcess inspect(L"--inspect "+quote(rejected.wstring()));auto response=inspect.readText({});check(!inspect.succeeded()&&response.find("MODEL_INVALID")!=std::string::npos,"real worker rejects invalid manifest schema before GPU/model loading");}
   ...
   {NrSession denied(rejected,std::string(64,'0'));check(until([&]{return denied.status().find("RESEARCH_IDENTITY_CHANGED")!=std::string::npos;}),"explicit research mode rejects mismatched model consent identity before GPU use");check(!denied.ready(),"invalid research consent cannot publish a ready state");}
