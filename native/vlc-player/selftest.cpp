...
// Exercises the shipped engine against real libVLC, not a fake player.
#include "engine.hpp"
#include <atomic>
#include <chrono>
#include <fstream>
...
struct Frames {
  ...
  static void* lock(void* opaque, void** planes) {
    auto* self = static_cast<Frames*>(opaque);
    self->mutex.lock(); *planes = self->pixels; return nullptr;
  }
  static void unlock(void* opaque, void*, void* const*) {
    auto* self = static_cast<Frames*>(opaque);
    double total = 0;
    for (unsigned y = 24; y < 64; ++y)
  ...
  void attach(Engine& engine) {
    engine.api->SetCallbacks(engine.player,lock,unlock,display,this);
    engine.api->SetFormat(engine.player,"RV32",160,96,160 * 4);
  }
};
bool until(const std::function<bool()>& fn,int milliseconds=5000) {
  const auto end=std::chrono::steady_clock::now()+std::chrono::milliseconds(milliseconds);
  do { if(fn()) return true; std::this_thread::sleep_for(std::chrono::milliseconds(25)); } while(std::chrono::steady_clock::now()<end);
  return fn();
}
std::string json(const std::string& s) {
  std::string out="\"";
  for(unsigned char c:s) { if(c=='"'||c=='\\')out+='\\'; if(c>=32)out+=static_cast<char>(c); }
  return out+'"';
}
}
int playbackTest(const std::filesystem::path& root,const std::filesystem::path& fixture,const std::filesystem::path& report) {
  ...
  try {
    ...
    { Engine engine(api,nullptr,false,true);frames.attach(engine);
      ...
      engine.open(item);
      check(until([&]{return frames.count>=8&&engine.length()>10000&&engine.seekable();}),"real AVI RGB video and PCM audio decoding, duration and seeking");
      ...
      check(api->EqCount()>0,"VLC equalizer presets available");engine.equalizer(0);engine.equalizer(-1);passed.emplace_back("equalizer enabled and disabled through libVLC");
      check(api->SetAudioDelay(engine.player,100000)==0,"audio timing adjustment accepted");
      engine.subtitle(fixture.parent_path()/L"captions.srt");
      check(until([&]{return engine.tracks(false).size()>1;}),"external SRT subtitle loaded and enumerated");
      check(api->SetSubtitleDelay(engine.player,150000)==0,"subtitle timing adjustment accepted");
      auto png=report.parent_path()/L"vlc-decoded-frame.png";
      check(engine.snapshot(png)&&until([&]{return std::filesystem::exists(png)&&std::filesystem::file_size(png)>64;}),"VLC displayed-frame PNG snapshot written");
      std::ifstream image(png,std::ios::binary);char signature[8]{};image.read(signature,8);
      check(std::string(signature,8)==std::string("\x89PNG\r\n\x1a\n",8),"snapshot has a valid PNG signature");
      engine.stop();check(api->State(engine.player)==libvlc_Stopped,"stop completes");
      const auto before=frames.count.load();engine.open(item);check(until([&]{return frames.count>before+3;}),"stop and reopen continue real decoding");
      // A second decoder is a user-imported comparison, not generated DLSS output.
      Frames second;
      {Engine imported(api,nullptr,true,true);second.attach(imported);imported.open(item);
       check(until([&]{return second.count>=3;}),"independent imported-video decoder produces frames");
       check(api->GetAudioTrack(imported.player)==-1,"comparison decoder has no selected audio stream");
       imported.stop();}
