...
int WINAPI wWinMain(HINSTANCE instance,HINSTANCE,PWSTR,int show){
 ...
 try{
  if(argc==5&&std::wstring(argv[1])==L"--self-test"){result=playbackTest(argv[2],argv[3],argv[4]);}
   ...
   MSG msg{};while(GetMessageW(&msg,nullptr,0,0)>0){try{if(window.key(msg))continue;}catch(const std::exception&e){window.failure(e);}if(!IsDialogMessageW(h,&msg)){TranslateMessage(&msg);DispatchMessageW(&msg);}}result=static_cast<int>(msg.wParam);
  }
