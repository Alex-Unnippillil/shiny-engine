...
LRESULT CALLBACK windowProc(HWND h,UINT m,WPARAM w,LPARAM l){
 ...
 case WM_MEASUREITEM:reinterpret_cast<MEASUREITEMSTRUCT*>(l)->itemHeight=static_cast<UINT>(p->units(60));return TRUE;
 case WM_SETTINGCHANGE:p->applyDpi(p->dpi);p->layout();return 0;
 case WM_NOTIFY:if(reinterpret_cast<NMHDR*>(l)->code==NM_CUSTOMDRAW)return p->drawSlider(reinterpret_cast<NMCUSTOMDRAW*>(l));break;
 case WM_ERASEBKGND:{RECT r{};GetClientRect(h,&r);FillRect(reinterpret_cast<HDC>(w),&r,p->theme.background);return 1;}
 case WM_CTLCOLORBTN:case WM_CTLCOLORSTATIC:case WM_CTLCOLORLISTBOX:case WM_CTLCOLOREDIT:{
  const int id=GetDlgCtrlID(reinterpret_cast<HWND>(l));const bool card=m==WM_CTLCOLOREDIT||m==WM_CTLCOLORLISTBOX||id==STATEBADGE||id==WELCOMETITLE||id==WELCOMEBODY||id==303;
  return reinterpret_cast<LRESULT>(p->theme.control(reinterpret_cast<HDC>(w),card));}
 case WM_PAINT:{PAINTSTRUCT ps;auto dc=BeginPaint(h,&ps);p->paintWorkspace(dc);EndPaint(h,&ps);return 0;}
 case WM_COMMAND:{int id=LOWORD(w);if(id==QUEUE){if(HIWORD(w)==LBN_DBLCLK){if(auto index=p->queueIndex())p->select(*index);}else if(HIWORD(w)==LBN_SELCHANGE){p->updateQueueControls();}}else if(id==QUEUESEARCH){if(HIWORD(w)==EN_CHANGE)p->refreshQueue();}else if(id==RATE){if(HIWORD(w)==CBN_SELCHANGE&&p->engine){const float rates[]={.5f,.75f,1.f,1.25f,1.5f,2.f};auto i=SendMessageW(p->rateBox,CB_GETCURSEL,0,0);if(i>=0&&i<6){if(p->api->SetRate(p->engine->player,rates[i])<0)throw std::runtime_error("Playback speed unsupported for this input.");p->playbackRate=rates[i];if(p->treatment)p->api->SetRate(p->treatment->player,rates[i]);}}}else p->action(id);p->updateWorkspace();return 0;}
 ...
 case WM_TIMER:if(IsWindowEnabled(h))p->tick();return 0;
