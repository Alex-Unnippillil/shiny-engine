...
LRESULT CALLBACK windowProc(HWND h,UINT m,WPARAM w,LPARAM l){
 ...
 case WM_GETMINMAXINFO:reinterpret_cast<MINMAXINFO*>(l)->ptMinTrackSize={1030,690};return 0;
 ...
 case WM_PAINT:{PAINTSTRUCT ps;HDC dc=BeginPaint(h,&ps);SetBkMode(dc,TRANSPARENT);SetTextColor(dc,text);auto old=SelectObject(dc,p->titleFont);RECT r{20,18,500,50};DrawTextW(dc,L"SHINY PLAYER",-1,&r,DT_SINGLELINE);SetTextColor(dc,muted);SelectObject(dc,p->font);r={225,25,920,50};DrawTextW(dc,L"0.6  /  LOCAL MEDIA + NATIVE NEURAL WORKBENCH",-1,&r,DT_SINGLELINE);SelectObject(dc,old);EndPaint(h,&ps);return 0;}
