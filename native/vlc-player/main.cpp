...
LRESULT CALLBACK windowProc(HWND h,UINT m,WPARAM w,LPARAM l){
 ...
 case WM_PAINT:{PAINTSTRUCT ps;HDC dc=BeginPaint(h,&ps);SetBkMode(dc,TRANSPARENT);SetTextColor(dc,text);auto old=SelectObject(dc,p->titleFont);RECT r{p->units(20),p->units(18),p->units(215),p->units(50)};DrawTextW(dc,L"SHINY PLAYER",-1,&r,DT_SINGLELINE);SetTextColor(dc,muted);SelectObject(dc,p->font);RECT client{};GetClientRect(h,&client);r={p->units(225),p->units(25),client.right-p->units(20),p->units(50)};DrawTextW(dc,L"0.8  /  LOCAL PLAYBACK · RESEARCH STUDIO",-1,&r,DT_SINGLELINE|DT_END_ELLIPSIS);SelectObject(dc,old);EndPaint(h,&ps);return 0;}
