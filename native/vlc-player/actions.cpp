...
void Window::action(int id){
 ...
 if(id==MANAGEDPREVIEW){
  if(!queue.selected||queue.items[*queue.selected].remote||!engine->seekable())throw std::runtime_error("Play a seekable local video before starting managed preview.");
  unsigned w=0,h=0;if(api->VideoSize(engine->player,0,&w,&h)||!w||!h)throw std::runtime_error("No decoded video dimensions yet.");
  managed.reset();neural.reset();managed=std::make_unique<ManagedPanel>(hwnd,api,queue.items[*queue.selected],w,h,engine->time());return;
 }
 if(id==NR){if(!queue.selected||queue.items[*queue.selected].remote||!engine->seekable())throw std::runtime_error("Play a seekable local video before opening Neural Workbench.");unsigned w=0,h=0;if(api->VideoSize(engine->player,0,&w,&h)||!w||!h)throw std::runtime_error("No decoded video dimensions yet.");managed.reset();neural.reset();neural=std::make_unique<NeuralPanel>(hwnd,api,queue.items[*queue.selected],w,h,engine->time());return;}
 ...
 if(id==LOOPB){loop.end(engine->time());message(L"A-B loop enabled.");return;}
