...
void Window::fullscreenToggle(){
 if(!fullscreen){placement.length=sizeof(placement);GetWindowPlacement(hwnd,&placement);oldStyle=GetWindowLongPtrW(hwnd,GWL_STYLE);MONITORINFO mi{sizeof(mi)};GetMonitorInfoW(MonitorFromWindow(hwnd,MONITOR_DEFAULTTONEAREST),&mi);SetMenu(hwnd,nullptr);SetWindowLongPtrW(hwnd,GWL_STYLE,oldStyle&~WS_OVERLAPPEDWINDOW);SetWindowPos(hwnd,topmost?HWND_TOPMOST:HWND_TOP,mi.rcMonitor.left,mi.rcMonitor.top,mi.rcMonitor.right-mi.rcMonitor.left,mi.rcMonitor.bottom-mi.rcMonitor.top,SWP_FRAMECHANGED);fullscreen=true;}
 else{SetWindowLongPtrW(hwnd,GWL_STYLE,oldStyle);SetMenu(hwnd,menu);SetWindowPlacement(hwnd,&placement);SetWindowPos(hwnd,topmost?HWND_TOPMOST:HWND_NOTOPMOST,0,0,0,0,SWP_NOMOVE|SWP_NOSIZE|SWP_FRAMECHANGED);fullscreen=false;}layout();
}
void Window::fullVlc(){
 if(!api)throw std::runtime_error("Locate an official VLC installation first.");const auto exe=(api->root/L"vlc.exe").wstring();std::wstring command=quote(exe)+L" --no-one-instance";
 ...
 STARTUPINFOW si{sizeof(si)};PROCESS_INFORMATION pi{};if(!CreateProcessW(exe.c_str(),command.data(),nullptr,nullptr,FALSE,0,nullptr,api->root.c_str(),&si,&pi))throw std::runtime_error("Could not launch the installed VLC interface.");CloseHandle(pi.hThread);CloseHandle(pi.hProcess);if(engine)engine->pause(true);if(treatment)treatment->pause(true);message(L"Full VLC opened separately. Shiny playback paused to prevent duplicate audio.");
}
void Window::refreshMenu(HMENU m){
 ...
 if(m==audioMenu||m==subMenu){clear();auto& tracks=m==audioMenu?audioTracks:subTracks;tracks=engine?engine->tracks(m==audioMenu):std::vector<std::pair<int,std::wstring>>{};int current=engine?(m==audioMenu?api->GetAudioTrack(engine->player):api->GetSubtitle(engine->player)):-99;
  for(size_t i=0;i<std::min<size_t>(999,tracks.size());++i)AppendMenuW(m,MF_STRING|(tracks[i].first==current?MF_CHECKED:0),(m==audioMenu?AUDIOFIRST:SUBFIRST)+i,tracks[i].second.c_str());if(tracks.empty())AppendMenuW(m,MF_STRING|MF_GRAYED,0,L"No tracks available yet");
...
void Window::effectsFromControls(){
 ...
 adjustments.contrast=static_cast<float>(SendMessageW(GetDlgItem(hwnd,CONTRAST),TBM_GETPOS,0,0))/100;adjustments.brightness=static_cast<float>(SendMessageW(GetDlgItem(hwnd,BRIGHTNESS),TBM_GETPOS,0,0))/100;adjustments.saturation=static_cast<float>(SendMessageW(GetDlgItem(hwnd,SATURATION),TBM_GETPOS,0,0))/100;adjustments.gamma=static_cast<float>(SendMessageW(GetDlgItem(hwnd,GAMMA),TBM_GETPOS,0,0))/100;
 if(engine){engine->effects=adjustments;if(treatment)engine->effects.enabled=false;engine->applyEffects();}message(treatment?L"Comparison preserves the source. Adjustments resume after closing comparison.":L"Live VLC image adjustments. Conventional filters, not DLSS or neural rendering.");
}
...
void Window::tick(){
 if(engine){auto st=api->State(engine->player);SetWindowTextW(GetDlgItem(hwnd,PLAY),st==libvlc_Playing?L"Pause":L"Play");
  ...
  if(treatment&&GetTickCount64()-lastSync>500){lastSync=GetTickCount64();auto ts=api->State(treatment->player);
   ...
   else if(ts==libvlc_Playing||ts==libvlc_Paused){if(engine->length()>0&&treatment->length()>0&&std::llabs(engine->length()-treatment->length())>1500){closeComparison();message(L"Comparison rejected: video durations differ by more than 1.5 seconds.");}else{if(syncPending||std::llabs(engine->time()-treatment->time())>120)treatment->seek(engine->time());treatment->pause(!engine->playing());syncPending=false;}}
  }
