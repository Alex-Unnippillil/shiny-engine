...
foreach ($path in @('ShinyVlcPlayer.exe','ShinyLibraryManager.exe','ShinyLibraryManagerCli.exe','ShinyEnhancementWorker.exe','library-bundles\1.0.0\shiny_spatial.dll','library-bundles\1.1.0\shiny_spatial.dll','nr\ShinyNrWorker.exe','runtime\vlc-3.0.24\libvlc.dll','unins000.exe')) { if (-not (Test-Path (Join-Path $target $path))) { throw "Installer omitted $path" } }
...
try {
  ...
  if ($LASTEXITCODE -ne 0 -or $catalog.packages.Count -ne 2) { throw 'Installed manager did not import its exact bundled versions' }
...
foreach ($path in @('ShinyVlcPlayer.exe','ShinyLibraryManager.exe','ShinyLibraryManagerCli.exe','ShinyEnhancementWorker.exe','library-bundles\1.0.0\shiny_spatial.dll','library-bundles\1.1.0\shiny_spatial.dll','nr\ShinyNrWorker.exe','runtime\vlc-3.0.24\libvlc.dll')) { if (Test-Path (Join-Path $target $path)) { throw "Uninstaller retained managed file $path" } }
@{ installed=$true; managedLibrarySelection=$true; managedWorkerProbe=$true; verifiedCachedVlc=$true; runtimeAutoDetected=$true; decodedVideoFrames=$ui.decodedVideoFrames; uninstalled=$true; managedLibrarySelection=$true; managedWorkerProbe=$true; trainedModelInference=$false; signed=$false; scope='Disposable Windows CI user. No physical GPU, neural model, online prerequisite download or sound device validation.' } | ConvertTo-Json | Set-Content (Join-Path $proof 'installer-report.json')
